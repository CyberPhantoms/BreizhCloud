/******************************************************************************
* Original server from Shen Feng, https://github.com/shenfeng/tiny-web-server
* Released under the MIT License
*******************************************************************************

#include "../librot13/rot13.h"
...
void process(int fd, struct sockaddr_in *clientaddr) {
  ...
  (void)rot13(req.filename);
  client_error(fd, 200, "OK", req.filename);
...
int main(int argc, char **argv) {
  ...
  const char *env_port = getenv("ROTAAS_PORT");
  if (env_port != NULL) {
    default_port = atoi(env_port);
    if (default_port == 0) {
      printf("Server port seems to be invalid: %s\n", env_port);
      return 1;
    }
  }
